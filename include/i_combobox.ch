...
#xcommand @ <row>, <col> COMBOBOX <name> ;
      ...
      [ <dummy: ONCHANGE, ON CHANGE> <changeprocedure> ] ;
      [ <dummy: ONLOSTFOCUS, ON LOSTFOCUS> <lostfocus> ] ;
      [ <dummy: ONENTER, ON ENTER> <enter> ] ;
      [ <dummy: ONDISPLAYCHANGE, ON DISPLAYCHANGE> <displaychng> ] ;
      [ <notabstop: NOTABSTOP> ] ;
      [ HELPID <helpid> ] ;
      [ <invisible: INVISIBLE> ] ;
      [ <dummy: IMAGE, IMAGES> <aImage> ] ;
      ...
      [ FONTCOLOR <fontcolor> ] ;
      [ LISTWIDTH <listwidth> ] ;
      [ <dummy: ONLISTDISPLAY, ON LISTDISPLAY> <listdisplay> ] ;
      [ <dummy: ONLISTCLOSE, ON LISTCLOSE> <listclose> ] ;
      [ <delay: DELAYEDLOAD> ] ;
      [ <incremental: INCREMENTAL> ] ;
      [ <winsize: INTEGRALHEIGHT> ] ;
      [ <rfrsh: REFRESH, NOREFRESH> ] ;
      [ SOURCEORDER <sourceorder> ] ;
      [ <dummy: ONREFRESH, ON REFRESH> <refresh> ] ;
      [ SEARCHLAPSE <nLapse> ] ;
      [ <NoTrans: NOLOADTRANSPARENT> ] ;
      [ <dummy: ONCANCEL, ON CANCEL> <cancel> ] ;
...
#xcommand COMBOBOX <name> ;
   ...
   => ;
      [ <obj> := ] _OOHG_SelectSubClass( TCombo(), [ <subclass>() ] ): ;
            Define( <(name)>, <(parent)>, NIL, NIL, <width>, <rows>, <value>, ;
            <fontname>, <fontsize>, <tooltip>, <{changeprocedure}>, <height>, <{gotfocus}>, ;
            <{lostfocus}>, <{enter}>, <helpid>, <.invisible.>, <.notabstop.>, ;
            <.sort.>, <.bold.>, <.italic.>, <.underline.>, <.strikeout.>, ;
