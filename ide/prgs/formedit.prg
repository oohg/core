/*
 * $Id: formedit.prg,v 1.53 2015-03-09 02:51:06 fyurisich Exp $
 */
/*
...
LOCAL i, j, nContLin, cForma, nStart, nEnd, nFWidth, nFHeight, cName, aColor
   ...
   FOR i := 1 TO nContLin
      aAdd( ::aLine, ' ' + MemoLine( cForma, 1200, i ) )
      IF ::aLine[i] # NIL
...
LOCAL cName, oCtrl, aImages, aItems, nMin, nMax, j, aCaptions, nCnt, oPage, lRed
   ...
   CASE nControlType == 7            // 'GRID'
      oCtrl := TGrid():Define( cName, ::oDesignForm:Name, ;
                  ...
                  .F., ! ::aNoHeaders[i], NIL, NIL, NIL, .F., NIL,  NIL, ::aCheckBoxes[i], ;
...
LOCAL nFRow, nFCol, nFWidth, nFHeight
   ...
   ::cFGripperText        := ::Clean( ::ReadStringData( 'WINDOW', "GRIPPERTEXT", '' ) )
...
LOCAL lUpdateColors, oCtrl, lNoModalEdit, lByCell
...
LOCAL aFormats, aResults
   ...
   IF ::aCtrlType[j] == 'EDIT'
      ...
      aFormats    := { 30,         120,           '999999',        .F.,            1100,        '999',        .F.,         250,         .F.,           .F.,           .F.,             .F.,             .F.,             31,         .F.,       .F.,          '99' }
      aResults    := ::myIde:myInputWindow( cTitle, aLabels, aInitValues, aFormats )
      IF aResults[1] == NIL
         ::oDesignForm:SetFocus()
         RETURN NIL
   ...
   IF ::aCtrlType[j] == 'RICHEDIT'
      ...
      aFormats    := { 30,         120,           '999999',        .F.,            1100,        '999',        .F.,         250,         .F.,           .F.,           .F.,             31,         .F.,             .F.,             '9',            .F.,       '99',             .F.,             .F.,             250,        250 }
      aResults    := ::myIde:myInputWindow( cTitle, aLabels, aInitValues, aFormats )
      IF aResults[1] == NIL
         ::oDesignForm:SetFocus()
         RETURN NIL
   ...
   IF ::aCtrlType[j] == 'GRID'
      ...
      aFormats    := { 30,         1100,          1100,         1100,        "999999",     250,           .F.,               .F.,           60,          '999',        .F.,         350,           .F.,           .F.,           350,                    350,                    1100,                 1100,        1100,            1100,       1100,            .T.,           1100,            .T.,        31,         .F.,           '9999',          .F.,             250,                250,               .F.,              250,             250,            .F.,                .F.,                .F.,             .F.,               .F.,               .F.,             .F.,               250,                 250,                250,                 250,                250,                 .F.,             .F.,          250,              250,             250,            .F.,               .F.,              .F.,                .F.,                  .F.,                   .F.,               .F.,              250 }
      aResults    := ::myIde:myInputWindow( cTitle, aLabels, aInitValues, aFormats )
      IF aResults[1] == NIL
         ::oDesignForm:SetFocus()
         RETURN NIL
   ...
   IF ::aCtrlType[j] == 'LABEL'
      ...
      aFormats    := { 30,         300,         '999',        .F.,               .F.,               .F.,              120,           .F.,            .F.,           .F.,           .F.,              .F.,          31,         1100,            .F.,             .F.,             .F.,       .F.,        .F.,            250 }
      aResults    := ::myIde:myInputWindow( cTitle, aLabels, aInitValues, aFormats )
      IF aResults[1] == NIL
         ::oDesignForm:SetFocus()
         RETURN NIL
   ...
   IF ::aCtrlType[j] == 'LIST'
      ...
      aFormats    := { 30,         '999',        1100,        120,           .F.,               '999',        .F.,         .F.,             .F.,        .F.,           .F.,           31,         .F.,       .F.,             250,         .F.,       '999',            250 }
      aResults    := ::myIde:myInputWindow( cTitle, aLabels, aInitValues, aFormats )
      IF aResults[1] == NIL
         ::oDesignForm:SetFocus()
         RETURN NIL
   ...
   IF ::aCtrlType[j] == 'XBROWSE'
      ...
      aFormats    := { 30,         1100,          1100,         80,             1100,         '999999',     250,           1100,        1100,             1100,            .T.,        .F.,          .F.,           1100,        1100,          '99999',      .F.,           .F.,           1100,       1100,                   1100,                   1100,                 1100,            .F.,           .F.,        .F.,          31,         .F.,         .F.,       .F.,             .F.,         .F.,           .F.,             250,                250,               250,              250,            .F.,                .F.,                .F.,             .F.,               .F.,               .F.,             .F.,               .F.,             250,              250,             .F.,            .F.,              .F.,                .F.,             .F.,             250,                 250,                250,                 250,                250,                 .F.,               .F.,             250,                250,            250,              .F.,            .F.,           .F.,                .F.,              .F.,               .F. }
      aResults    := ::myIde:myInputWindow( cTitle, aLabels, aInitValues, aFormats )
      IF aResults[1] == NIL
         ::oDesignForm:SetFocus()
         RETURN NIL
...
LOCAL ia, aResults
   ...
   IF ::aCtrlType[j] == 'GRID'
      cTitle      := cNameW + " events"
      aLabels     := { 'On Change',    'On GotFocus',    'On LostFocus',    'On DblClick',    'On Enter',    'On Headclcik',    "On EditCell",    "On QueryData",    "On AborEdit",     "On Delete",    'On CheckChange', 'Action',     'On Append',    "On HeadRClick" }
      aInitValues := { ::aonchange[j], ::aongotfocus[j], ::aonlostfocus[j], ::aondblclick[j], ::aOnEnter[j], ::aonheadclick[j], ::aoneditcell[j], ::aOnQueryData[j], ::aOnAbortEdit[j], ::aOnDelete[j], ::aOnCheckChg[j], ::aAction[j], ::aonappend[j], ::aOnHeadRClick[j] }
      aFormats    := { 250,            250,              250,               250,              250,           250,               250,              250,               250,               250,            250,              250,          250,            250 }
      aResults    := ::myIde:myInputWindow( cTitle, aLabels, aInitValues, aFormats )
      IF aResults[1] == NIL
         ::oDesignForm:SetFocus()
         RETURN NIL
