/*
 * $Id: formedit.prg,v 1.25 2014-08-13 23:36:13 fyurisich Exp $
 */
   ...
   IF ! IsWindowDefined( Form_1 )
      ...
      DEFINE WINDOW Lista OBJ Lista ;
         ...
         DEFINE CONTEXT MENU CONTROL ListaCon OF Lista
            ...
            ITEM 'Global Vert Gap Change' ACTION GlobalVertGapChg( oListaCon, ::myIde )
            SEPARATOR
            ITEM 'Print Brief'            ACTION PrintBrief( oListaCon )
            SEPARATOR
            ITEM 'Delete'                 ACTION DeleteControl()
         END MENU
...
*------------------------------------------------------------------------------*
STATIC FUNCTION PrintBrief( oListaCon )
*------------------------------------------------------------------------------*
   LOCAL aInput, aItems, aControls, i, nFila, nRow, nCol, nHeight, nWidth, uValue, cName, oCtrl, cRealName, cType, oPrint, contlin, wpage, lDynamicOnly, nIndice
   LOCAL aOptions  := {'Name','Type','Row','Col'}
   LOCAL aOptionsN := {1,7,2,3}

   aItems       := {}
   aInput       := myInputWindow( 'Print Brief', { 'Filter Dynamic Controls','Print Events', 'Sort By' }, { .T., .F., 3 }, { NIL , NIL, aOptions } )
   DEFAULT aInput[3] TO 3
   lDynamicOnly := aInput[1]
   oForm        := GetFormObject( 'Form_1' )
   nCantItems   := oListaCon:ItemCount
   CursorWait()

   FOR i:=1 TO nCantItems
      AADD(aItems,{oListaCon:cell(i,1),oListaCon:cell(i,2),oListaCon:cell(i,3),oListaCon:cell(i,4),oListaCon:cell(i,5),oListaCon:cell(i,6),oListaCon:cell(i,7)})
   NEXT
   nIndice      := aOptionsN[aInput[3]]
   aSort( aItems, , , { |x, y| x[nIndice] < y[nIndice] } )
   ...
   contlin := 1
   wpage   := 1
   oprint:printdata(contlin,0,'BRIEF OF '+IIF(lDynamicOnly,'DYNAMIC','ALL')+' CONTROLS IN FORM: '+oForm:title+'  OBJ: '+'(data not available)')
   contlin++
   oprint:printdata(contlin,0,'FILE: '+form_main:frame_1:caption)
   contlin++
   oprint:printdata(contlin,0,'SORTED BY '+UPPER(aOptions[aInput[3]]))
   contlin++
   oprint:printdata(contlin,0,replicate('-',120))
   contlin++
   oprint:printdata(contlin,0,'NAME                    TYPE        ROW    COL   WIDTH HEIGHT  VALUE                 OBJ')
   ...
   FOR i := 1 TO nCantItems
      ...
      uValue    := oCtrl:Value
      nIndice   := ascan(myform:acontrolw,{ |c| lower( c ) == lower(cName) } )
      cObj      := myform:acobj[nIndice]
      ...
      if (upper(cType)$"LABEL FRAME TIMER IMAGE") .and. lDynamicOnly
         ...
         ENDIF
         IF aInput[2]
            IF !EMPTY(myform:aaction[nIndice])
               oprint:printdata(++contlin,005,'ACTION     : '+ALLTRIM(CSTR(myform:aaction[nIndice])))
            ENDIF
            IF !EMPTY(myform:aaction2[nIndice])
               oprint:printdata(++contlin,005,'ACTION2    : '+ALLTRIM(CSTR(myform:aaction2[nIndice])))
            ENDIF
            IF !EMPTY(myform:aondblclick[nIndice])
               oprint:printdata(++contlin,005,'ONDBLCLICK : '+ALLTRIM(CSTR(myform:aondblclick[nIndice])))
            ENDIF
            IF !EMPTY(myform:aonchange[nIndice])
               oprint:printdata(++contlin,005,'ONCHANGE   : '+ALLTRIM(CSTR(myform:aonchange[nIndice])))
            ENDIF
            IF !EMPTY(myform:aongotfocus[nIndice])
               oprint:printdata(++contlin,005,'ONGOTFOCUS : '+ALLTRIM(CSTR(myform:aongotfocus[nIndice])))
            ENDIF
            IF !EMPTY(myform:aonlostfocus[nIndice])
               oprint:printdata(++contlin,005,'ONLOSTFOCUS: '+ALLTRIM(CSTR(myform:aonlostfocus[nIndice])))
            ENDIF
            IF !EMPTY(myform:aonenter[nIndice])
               oprint:printdata(++contlin,005,'ONENTER    : '+ALLTRIM(CSTR(myform:aonenter[nIndice])))
            ENDIF
            IF !EMPTY(myform:aondisplaychange[nIndice])
               oprint:printdata(++contlin,005,'ONDISP.CHG.: '+ALLTRIM(CSTR(myform:aondisplaychange[nIndice])))
            ENDIF
            IF !EMPTY(myform:aonheadclick[nIndice])
               oprint:printdata(++contlin,005,'ONHEADCLICK: '+ALLTRIM(CSTR(myform:aonheadclick[nIndice])))
            ENDIF
            IF !EMPTY(myform:aoneditcell[nIndice])
               oprint:printdata(++contlin,005,'ONEDITCELL : '+ALLTRIM(CSTR(myform:aoneditcell[nIndice])))
            ENDIF
            IF !EMPTY(myform:aonappend[nIndice])
               oprint:printdata(++contlin,005,'ONAPPEND   : '+ALLTRIM(CSTR(myform:aonappend[nIndice])))
            ENDIF
         ENDIF
      endif
   NEXT
